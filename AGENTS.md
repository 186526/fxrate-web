...
技术栈：Next.js 16.3（App Router，`output: "standalone"`）+ React 19 + TypeScript + MUI v6（`@mui/material`，含 `@mui/icons-material`）+ Emotion + Tailwind。
...
componets/           # 注意：目录名拼写如此（components 的 typo），是有意为之，勿改名
  ...
  ssr-prefetch.ts    # 默认视图 SSR 预取（server-only）：服务端 SWR 缓存（TTL 45s）+ 8s 超时降级，供 page.tsx 调用
...
- **数据流（客户端驱动 + 默认视图 SSR 预取）**：`app/page.tsx` 对**默认视图**（`/` 且参数为缺省或默认 from=CNY/to=USD/amount=100/precision=4）做 SSR 预取——经 `componets/ssr-prefetch.ts` 调 `showCurrencyAllRates()` → 串行 `getFXRateClient().info()` → `getCurrenciesDetails()`，结果以 `initialCurrencies`/`initialResult`（string 时间戳）/`initialBackendVersion` props 随 RSC 下发，`Index` 的 useState 初始化与 `pairCacheRef`/`pairSnapshot` 直接消费（`updated` 经 `new Date()` 转回），首屏直接渲染数据、浏览器 hydration 不再重复拉货币列表/版本；预取另有**服务端模块级 SWR 缓存**（TTL 45s，key 为参数签名）与 **8s `withTimeout` 超时**——任何失败/超时/空结果都降级为薄壳（客户端照常自己拉数），绝不阻塞首屏。**非默认参数/矩阵视图保持纯客户端**（历史卡顿根因 = 每次 URL 变化都触发整轮服务端重拉，因此 URL 参数变化绝不触发服务端数据请求）；`/matrix` 路由仍是薄壳。预取后客户端仍会按既有机制在挂载后 300ms 触发一次 SWR 刷新（补 visa 慢源行 + 最新值）。k8s 中 web 部署的 `FXRATE_API` 指向集群内 `http://fxrate:8080/v1/jsonrpc` 让 SSR 直连后端（`FXRATE_PROXY` 保持公网 URL 供浏览器 `/api/fxrate` 代理）。
...
- **矩阵视图**：`?from=X` 使用后端正向汇率（买入方向），`?to=X` 使用后端 `reverse=true` 的反向汇率（卖出方向），前端不做算术倒数；默认只显示 12 个常用币种（`fxmatrixgrid.tsx` 的 `DEFAULT_COMMON_CURRENCIES`），「显示货币」弹层可勾选启用其他，localStorage（key `fxrate-matrix-currencies`）持久化；「中间价/现钞/现汇」ToggleButtonGroup 切换列值，「最优价」弹层控制参与高亮的来源。主数据与单独补查行按**单元格字段级深合并**（`mergeCellRows`）：主数据对实际存在的重叠字段优先，补查独有字段（cash/remit/path/updated）保留，`false`/`0`/字符串等合法值不受影响。额外行以 **keyed 快照**（`{key, data}`）存储，key 对应当前请求参数——参数变化时渲染先按 key 隔离旧行（新参数绝不渲染旧行），异步响应写回要求 key 与已提交的 `rowParamsRef`（`useLayoutEffect` 同步）一致；请求 key 只随主数据/启用币种变化，额外行新增货币列不触发整批重置。自动补查（MasterCard 等空行源）按来源维护 `idle/loading/success/error` 状态：失败渲染「加载失败，重试」行可手动重试，失败不被永久去重（参数变化或主数据更新会重新评估）。**unchanged-key AbortError 复位**：StrictMode 双挂载 cleanup 会在 `client.batch()` 前提前 abort 自动补查请求，`handleLoadSource` 的 catch 对「参数未变（`rowParamsRef.current == requestKey`）」的 AbortError 复位源状态（删除 loading）与 `autoAttemptedRef` 去重记录，让 auto-load effect 重新评估并重试；参数已变的取消仍静默忽略（rowParamsKey 变化 effect 统一清理）。
...
- **构建元数据注入**：`next.config.mjs` 在构建开始时一次性冻结 `FXBUILD_ID`（CD 注入完整 commit，本地 production/dev 分别回落 `production`/`development`）、`FXBUILD_TIME`（CD 可注入，否则生成 ISO 时间）与 `FXRATE_PROXY_BUILD`（和 rewrites 使用同一个 `FXRATE_PROXY` 构建期解析值）；`app/page.tsx`/`app/matrix/page.tsx` 只读取这些内联常量传给 Index/Footer，不在请求运行时执行 git/读取 `.git`。`/api/backend-meta` 同样读取 `FXRATE_PROXY_BUILD`，因此报告的 RPC/REST 地址永远与 standalone 中已固化的 `/api/fxrate`/`/api/rest` rewrite 一致，运行时修改 `FXRATE_PROXY` 不会制造“文档地址已变但实际代理未变”的漂移。
- **响应安全头**：`next.config.mjs headers()` 对全部路由统一加 `X-Content-Type-Options: nosniff`、`X-Frame-Options: DENY`、严格来源 Referrer Policy 与禁用 camera/geolocation/microphone 的 Permissions Policy；`proxy.ts` 的 production nonce CSP 另含 `frame-ancestors 'none'`，并分别用 `x-fx-release`/`x-fx-build-time` 暴露构建标识与时间（不再把时间误作 release ID）。
- **已知问题（勿"顺手修"）**：
...
- **端到端（`test/e2e/`）**：Playwright + @axe-core/playwright。`playwright.config.ts` 的 webServer 先起本地 mock JSON-RPC 后端（`test/e2e/mock-server/index.cjs`，`/v1/jsonrpc` + `/__counters` + `/__reset`），再以 `FXRATE_API`/`FXRATE_PROXY` 指向它启动 `next dev`——SSR 与浏览器代理都走 mock，测试零真实上游依赖。**baseURL 必须用 `localhost` 而非 `127.0.0.1`**：Next 16 dev 的 allowedDevOrigins 会拦截 127.0.0.1 Host 的 dev 资源，导致 React 不水合。安全硬化：mock 与 next dev 都只绑定 loopback（mock `listen(PORT, "127.0.0.1")`、next 用 `-H 127.0.0.1`，不暴露局域网/公网），mock 请求体上限 1 MiB（超限 413 且不计批次）；端口可用 `MOCK_PORT`/`WEB_PORT` 环境变量覆盖避免撞端口。`test/e2e/helpers/mockJsonRpc.ts` 拦截浏览器端 `/api/fxrate` 做确定性应答与请求计数（含 `paramsOf(method)` 供方向/参数断言），可选 `{ hold: [method] }` 持有每个 method 第一个命中批次直到 `release(method)` 放行（确定性控制响应时序）；断言语义：**默认视图 SSR 预取 + 浏览器 hydration 不重复拉**——`request-count.spec.ts`：首屏 pair 浏览器层 = 主批量（3 快源 ×2 方向）+ visa 慢源后台 = 2 条批量、8 个 getFXRate、instanceInfo/listCurrencies 恒为 0（SSR 已随 RSC 提供）；切矩阵（/matrix 薄壳无 initial props）浏览器 currenciesCache 冷 + dev StrictMode 双挂载 → showCurrencyAllRates 双份（2×2 批量）+ 版本 info + 矩阵 = +6 批量；切回 pair 命中服务端 SWR 缓存 + 浏览器 LRU → 数据零新增。`navigation-race.spec.ts` 用相对增量断言（切货币/金额 = +2 批量、+8 getFXRate、零 document 导航），SSR 下不受影响。`route-loading.spec.ts` 不依赖非保证的 loading.tsx 边界：`mockJsonRpcRoutes({ hold: ["listFXRates"] })` 持有浏览器 listFXRates 批次，断言矩阵客户端骨架（`role=status`「正在加载汇率数据」含货币列头、无单对列头）在数据到达前保持，`release` 后骨架被真实表格替换。`matrix-reverse.spec.ts` 覆盖矩阵反向（`?to=` 方向 URL、反向文案、零 document 导航、listFXRates 带 `reverse=true`、直接加载 footer 显示 `后端 fxrate@mock` 且浏览器层 instanceInfo 发生）；`mock-server.spec.ts` 覆盖 loopback 可达与 413（只直连 mock、不经页面，不受 SSR 影响）；`scroll-corner.spec.ts` 覆盖矩阵双轴滚动（窄视口 + 容器限高制造横/纵滚动）：角落格钉在容器左上角、`zIndex > 2`、`elementFromPoint` 命中角落（表头货币列不盖住交叉区），附截图；`api-docs.spec.ts` 的「试试看」点击一律先等 `#<endpoint-id>` 工作台挂载（`#rpc-get-rate` 只在 hydration + hash 同步后存在，SSR 只渲染默认端点）再把点击限定在 `workbench` 内，避免点击落在 SSR 瞬态按钮上把请求发往错误端点（light axe 一次偶发失败根因）；a11y gate 断言 serious+critical，白名单仅放行既有对比度节点（表头 muted 已加深为 `#5a666e` 达 AA 后移除表头/ToggleButton 条目；仍放行 RelativeTime 与 footer 版本 caption，见文件内证据注释）。
...
- CI 门禁：`.github/workflows/ci.yml` 对每个分支 push/PR 跑 `npx tsc --noEmit` + `yarn lint`（非变更性，`eslint .` 无 `--fix`）+ `yarn test`（vitest 单元测试）+ `yarn build`（next build 生产构建），checkout 需 `submodules: recursive`（tsc 要解析 `lib/fxrate/src/client`）。CI/CD 第三方 actions 全部锁到不可变 commit SHA，Node 固定 24.15.0。ci.yml 另含 `workflow_call` 触发——Phase 6 发布门禁：`.github/workflows/cd.yml` 的 `build-smoke-push` job `needs: gates`（复用同一组 CI 门禁，任一失败发布即被阻断），仅 release tag（v*）触发；构建前强制 `v<版本> == package.json.version`，并把 `github.sha`/UTC ISO 时间作为 `FXBUILD_ID`/`FXBUILD_TIME` build args 注入。顶层 `concurrency` 按 ref 串行（release: published 与 push: tags 同 ref 不竞态）。**两级 smoke + 推精确测试产物**：① 构建发布镜像（`build-push-action` push:false + load，**无 `FXRATE_PROXY` build-arg → 生产默认代理**）；② Tier-1 `scripts/image-smoke.sh --exact --image <tag> --expected-proxy <生产地址>` 对「精确发布镜像」跑确定性运行时 smoke（前端 `/` + 安全/构建响应头 + `/api/backend-meta` 精确生产 proxy + 容器存活；只验契约形状，**不访问真实银行**）；③ Tier-2 默认 `scripts/image-smoke.sh` 构建**契约镜像（不发布）**——`--build-arg FXRATE_PROXY` 指向 smoke 后端，并注入确定性 smoke build 元数据，验证后端 `/info`、`/readyz`、`/metrics` 8 family、JSON-RPC `instanceInfo`、`/api/backend-meta` 与浏览器代理 `/api/fxrate`（证明 build-arg 生效且元数据/代理一致；绝不把内部 smoke hostname 烤进发布镜像）；④ 对本地 daemon 中**已被 Tier-1 smoke 的同一批 tag** 逐个 `docker push`（绝不重新构建），再生成 attestation。镜像 tag：`:版本` + `:主.次` + `:sha-…`，非预发布 v*（无 `-`）额外 `:latest`。`scripts/image-smoke.sh` 支持 `--local`（无 Docker：`node lib/fxrate/dist/index.cjs` + `next dev`，跑 Tier-2 同一组断言）供本地预检——注意 `next dev` 同项目目录同时只能起一个（.next/dev 锁），与 e2e 并发时请用 docker contract 模式。
- Dockerfile：基础 `node:24-alpine` 锁定多架构 manifest digest；`.dockerignore` 排除 `.git`，builder 仅接收 `FXRATE_PROXY`/`FXBUILD_ID`/`FXBUILD_TIME` build args，runner 不安装 git、不复制仓库历史、也不保留运行时 `FXRATE_PROXY`。`next.config.mjs` rewrites 的 `/api/fxrate` 代理目标在 `next build` 时固化进 standalone，故镜像 smoke / 部署必须在构建期注入自定义 `FXRATE_PROXY`；不传时自动回落生产默认地址。
