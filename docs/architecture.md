...
`AGENTS.md` 指向本文档。改动数据流、视图缓存、慢源拆分、AbortSignal、矩阵合并与补查、URL 同步、主题与 CSP、构建元数据注入或测试基础设施之前读它。
...
- **国旗 emoji 字体（Windows）**：`sourceIcon.tsx` 的 `currencyEmoji()` 返回区域指示符国旗，取值集合封闭在 `country-locale-map` 的全部国家 emoji 加 `PRIMARY_FLAGS` 之内。Windows 的 Segoe UI Emoji 不含国旗字形（只渲染成两个字母），因此 `globals.css` 声明 `@font-face`（`public/fonts/noto-color-emoji-flags.woff2`，取自 Google Fonts 的 Noto Color Emoji 区域指示符分片，`unicode-range: U+1F1E6-1F1FF`，约 693 KB，含全部国旗），`layout.tsx` 只在请求头 `user-agent` 命中 `Windows NT` 时把该字体族追加到 body 的 `font-family`（用 inline style，优先级高于 CssBaseline，否则会被 `typography.fontFamily: "inherit"` 覆盖）；其他平台不引用该字体族即不下载，首屏不付这份体积。该分片无法再按货币裁剪：676 面国旗共用 26 个区段符号输入字形，子集化的 GSUB 闭包必然把整块拉回。**这是按 UA 分叉的 HTML，nginx 缓存键必须带 UA 变体**（`02-fxrate-openresty.yml` 的 `$fx_ua_variant`）：缓存键不含 UA 时，任一非 Windows 客户端先请求就会把「无字体族」的 body 写入缓存并交给之后所有 Windows 用户，国旗恒不显示，而 `@font-face`、字体文件、CSP `font-src 'self'` 全部正常——排查时容易误判。改完 ConfigMap 需 `rollout restart deployment/fxrate-ngx` 才重载（配置更新不触发滚动）。
...
- **对外展示的 REST 地址**：`/api/rest` 只是前端同源代理前缀（`next.config.mjs` rewrites），只在当前前端部署下可访问；后端自身挂在 `/`（如 `https://fxrate.sunoaki.net/rss/USD/CNY`）。API 文档页的 RSS 链接（`api-docs/reference.tsx`）与 curl（`api-docs/workbench.tsx`）取 `FXRATE_PROXY_BUILD` 去掉 `/v1/jsonrpc` 后缀的公开基址，单对视图的「复制 RSS 链接」（`tools.ts` 的 `rssURL()`）同法；三者取不到或后缀不匹配时回落同源 `/api/rest`。浏览器 bundle 能拿到 `FXRATE_PROXY_BUILD` 是因为 `next.config.mjs` 的 `env` 块把该常量内联进客户端（与 `FXBUILD_ID` 同机制）。
...
- **CI 门禁**（`.github/workflows/ci.yml`）：push/PR 跑 `npx tsc --noEmit` + `yarn lint`（非变更性，`eslint .` 无 `--fix`）+ `yarn test` + `yarn build`；checkout 需 `submodules: recursive`（tsc 要解析私有 `lib/fxrate/src/client`）并显式传 `secrets.FXRATE_SUBMODULE_TOKEN`——该 secret 必须是仅授权私有仓库 `186526/fxrate`、权限仅 `Contents: read` 的 fine-grained PAT，CI 与 CD 两处 checkout 共用，勿回落默认 `GITHUB_TOKEN`（公开父仓库 token 无跨仓库读取权限）；第三方 actions 锁不可变 commit SHA，Node 固定 24.15.0。`workflow_call` 供 Phase 6 发布门禁（`cd.yml` `build-smoke-push` `needs: gates`，仅 v* tag 触发；构建前强制 `v<版本> == package.json.version`，注入 `FXBUILD_ID`/`FXBUILD_TIME`；顶层 concurrency 按 ref 串行）。**两级 smoke + 推精确测试产物**：① 构建发布镜像（push:false + load，**无 `FXRATE_PROXY` build-arg → 生产默认代理**）；② Tier-1 `image-smoke.sh --exact --image <tag> --expected-proxy <生产地址>` 对精确发布镜像跑确定性 smoke（前端 `/` + 安全/构建响应头 + `/api/backend-meta` 精确 proxy + 容器存活，只验契约形状**不访问真实银行**）；③ Tier-2 默认构建**契约镜像（不发布）**（`--build-arg FXRATE_PROXY` 指向 smoke 后端 + 确定性元数据），验后端 `/info`、`/readyz`、`/metrics` 8 family、`instanceInfo`、`/api/backend-meta` 与浏览器代理 `/api/fxrate`；④ 对已 Tier-1 smoke 的**同一批 tag** 逐个 `docker push`（绝不重新构建）+ attestation。tag：`:版本` + `:主.次` + `:sha-…`，非预发布 v* 额外 `:latest`。`image-smoke.sh --local`（无 Docker：`node lib/fxrate/dist/index.cjs` + `next dev`，跑 Tier-2 同组断言）供本地预检——`next dev` 同目录只能起一个（.next/dev 锁），与 e2e 并发用 docker contract 模式。
